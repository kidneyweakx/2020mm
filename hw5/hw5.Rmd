...
TEJ <- TEJ %>% mutate(TOFV = (Buy+Sell),MthDate = ymd(TEJ$MthDate))
TEJ <- TEJ %>% select(MthDate,code,TOFV)
```
...
cat("Order flows", mean(a$OdrImb),"return", mean(a$Volatility))
cat("Order flows", mean(b$OdrImb),"return", mean(b$Volatility))
cat("Order flows", mean(c$OdrImb),"return", mean(c$Volatility))
### 波動度:b台塑 > a台積電 > c富邦金
