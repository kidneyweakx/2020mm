...
ohis <- function(bot, update) {
  ...
  orstr <- do.call(paste,lapply(1:nrow(orhis),function(t){
    paste("[",orhis$side[t],"]",orhis$market[t],"=> id:",orhis$id[t],
          "\n價:",orhis$price[t],"量:",orhis$vol[t],"實際:",orhis$rvol[t],
          "\n===================\n")
  }))
  orstr <- paste("[現有訂單]\n", orstr)
...
answer_cb <- function(bot, update) {
  ...
  if(word(data,2) %in% m){
    ...
    bot$sendMessage(chat_id = cid, text = "沒有這種市場")
  }
...
updater <- updater + 
  ...
  CommandHandler("this", this, pass_args = FALSE, filter_user) +
