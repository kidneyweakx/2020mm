...
sendmsg <- function(texts="*懶狗該交易囉!*"){
  bot$sendMessage(Sys.getenv("TG_ME"),
                  text = texts,
                  parse_mode = "Markdown"
  )}
sendpic <- function(path="./tmp.png"){
  bot$sendPhoto(Sys.getenv("TG_ME"),
                photo = path
  )}
...
while(1){
  ...
  tryCatch(k <- k %>% tq_mutate(select = c(close),
                       mutate_fun = SMA,
                       n = 10) %>%
    rename(MAF = SMA) %>%
    tq_mutate(select = c(close),
              mutate_fun = SMA,
              n = 30) %>%
    tq_mutate(select = close, mutate_fun = BBands, n=26, sd=2),error=function(e){
      sendmsg(e)
    })
  if(nth(k$dn,-2) > nth(k$close, -2) & last(k$dn)  <=  last(k$close)  & hold==0 |   # 下往上穿越下線：可能短期會反轉
     nth(k$mavg,-2) > nth(k$close,-2) & last(k$mavg) <= last(k$close) & hold==0 ) { # 下往上穿越中線：可能會加速向上
    print("進場")
    # 進場
    # ==========交易============
    ...
    Sys.sleep(3)
    #===========================
  } else if(nth(k$up,-2) < nth(k$close, -2) & last(k$up)  >=  last(k$close)  & hold==1 | # 上往下跌破上線：暗示上漲趨勢結束
            nth(k$mavg,-2) < nth(k$close,-2) & last(k$mavg) >= last(k$close) & hold==1 ){ # 由上往下跌破中線：可能會下跌
    print("出場")
    #  出場
