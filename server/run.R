...
ticker = ticker*2
while(1){
  tryCatch(k <- getk(market,5,1000),error = function(e) sendmsg("getK error!!!!"))
  ...
  if(last(k$MA120) >= last(k$close) & hold==0){
    ...
    Sys.sleep(3)
    #===========================
  } else if((last(k$MAF) <= last(k$MAS)) & nth(k$MAF, -2) > nth(k$MAS, -2) & last(k$close) > ticker*1.002  & hold == 1 ){
    print("出場")
    #  出場
